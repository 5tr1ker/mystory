    ...
    return (
        arrs.map(commit => (
            <div className="userPostCommit" key={commit.commentId}>
                <span className='commitnickname'><img src={commit.writerImage} alt="img" onError={(event) => {event.target.src = defaultALT}}  className="profileImageData"/>{commit.writer}</span>
