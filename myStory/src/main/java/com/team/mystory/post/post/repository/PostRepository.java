...
public interface PostRepository extends JpaRepository<FreePost, Long> , CustomPostRepository {
	...
	@QueryHints(value = @QueryHint(name = "org.hibernate.readOnly" ,value = "true"))
	@Query(value = "SELECT new com.team.mystory.post.post.dto.ReturnPostDataDTO(f.numbers , f.title , f.writer , f.postTime , f.likes , f.views , COUNT(c)) FROM FreePost f LEFT OUTER JOIN f.freeCommit c group by f.numbers order by f.numbers DESC")
